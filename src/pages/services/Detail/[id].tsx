...
export const getServerSideProps: GetServerSideProps<any> = async ({
  ...
  try {
    ...
    const response: any = await axios.get(
      `http://172.187.153.193:8084/serviceInventory_svc/pb/IndustryServices/GetIndustryServiceByIndustryId?id=${params?.id}`
    );
...
function details(props: any) {
  return (
    <div>
      {/* <p>{JSON.stringify(props.data.services)}</p>
      {props.data.services.map((item: any) => {
        return (
          <a href={`/services/childservices/${item.industryId}`}>
            {item.serviceName}
          </a>
        );
      })} */}
      ...
      <div className="col-md-12 px-5">
        <div className="row">
          {props?.data?.services?.map((itm: any, index: any) => (
